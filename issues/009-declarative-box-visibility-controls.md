...
- [ ] Declare stable box IDs, toggle keys, action IDs and displayed hints together; validate unique keys, existing targets and handler completeness. Go supplies small explicit action handlers.
- [ ] Hide and restore each box; surviving boxes use available space in wide and slim layouts. Support both hidden with intact title/status, and restore without a restart.
- [ ] Status/title hints reflect current visibility and remain reachable when every box is hidden. Hidden children receive no input or focus; restoring does not reset clock or other application state.
- [ ] One-shot honors declared initial visibility; watch toggles and quit work through the same bounded action mapping.
